...
class PhotoFactory(factory.django.DjangoModelFactory):
    ...
    status = fuzzy.FuzzyChoice(
        choices=[x[0] for x in Photo.STATUS_CHOICES], getter=lambda c: c[0]
    )
    img = ImageField(
        width=1000,
        height=1000,
        filename="sample.jpg",
        color=fuzzy.FuzzyChoice(
            ["blue", "yellow", "green", "orange", "red", "wheat", "olive"]
        ),
    )
    ...
    class Params:
        approved = factory.Trait(
            ...
            pub_date=LazyAttribute(
                lambda _: timezone.now() - timezone.timedelta(hours=1)
            ),
        )
        ...
        on_moderation = factory.Trait(status=Photo.ON_MODERATION, pub_date=None)
